...
export const productsAPI = {
  ...
  deleteImage: (id, imageType) => 
    api.delete(`/products/${id}/images/${imageType}`).then(res => res.data),
  ...
  // Get product by SKU
  getProductBySku: (sku) => 
    api.get(`/products/sku/${sku}`).then(res => res.data),
  ...
  // Upload CSV
  uploadCSV: (formData) => 
    api.post('/products/upload/csv', formData, {
      headers: { 'Content-Type': 'multipart/form-data' }
    }).then(res => res.data),

  // Get all unique tags
  getAllTags: () => 
    api.get('/products/tags').then(res => res.data),
  ...
  getLowStockProducts: (limit = 10) => 
    api.get('/products/low-stock', { params: { limit } }).then(res => res.data),

  getRealtimeStats: async () => {
    const response = await api.get('/products/realtime-stats');
  ...
  exportExcel: () => 
    ...
    api.post('/imports/products', formData, {
      headers: { 'Content-Type': 'multipart/form-data' }
    }).then(res => res.data),
};